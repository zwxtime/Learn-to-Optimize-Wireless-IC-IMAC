...
def generate_Gaussian(K, num_H, Pmax=1, Pmin=0, seed=2017):
    ...
    Pini = Pmax*np.ones(K)
    var_noise = 1
    X=np.zeros((K**2,num_H))#存储信道数据，K*K的矩阵展平，然后25000个样本
    Y=np.zeros((K,num_H))#Y存储WMMSE输出
    # Generate Channel Data
    total_time = 0.0#计时开始
    for loop in range(num_H):
        CH = 1/np.sqrt(2)*(np.random.randn(K,K)+1j*np.random.randn(K,K))#K*K的复数矩阵构成高斯IC信道
        H=abs(CH)
